...
int DIOController::initDIO(SPIDriver& spi, GPIODriver& gpio) {
...
void DIOController::enablePin(SPIDriver& spi, GPIODriver& gpio, DIOPinInfo pin) {
...
void DIOController::disablePin(SPIDriver& spi, GPIODriver& gpio, DIOPinInfo pin) {
    // Finds CS location of the secondary on the primary.
    int primaryRegAddress = pin.primaryPin < 8 ? OLATA : OLATB;
    int primaryValue = ~(0b00000001 << (pin.primaryPin % 8));
...
void DIOController::primaryWrite(SPIDriver& spi, GPIODriver& gpio, uint8_t regAddress, uint8_t value) {
    uint8_t data[4];
...
void DIOController::primaryWrite(SPIDriver& spi, GPIODriver& gpio, uint8_t regAddress, uint8_t value, int CS) {
    uint8_t data[4];
    data[0] = PRIMARY_WRITE_OPCODE;
    data[1] = regAddress;
    data[2] = value;
...
uint8_t DIOController::primaryRead(SPIDriver& spi, GPIODriver& gpio, uint8_t regAddress, int CS) {
    uint8_t data[4];
...
void DIOController::secondaryWrite(SPIDriver& spi, GPIODriver& gpio, uint8_t regAddress, uint8_t value) {
    uint8_t data[4];
...
uint8_t DIOController::secondaryRead(SPIDriver& spi, GPIODriver& gpio, uint8_t regAddress) {
    uint8_t data[4];
